...
def run(page, port, label, expect_mode, expect_png_basemap):
    ...
    page.evaluate("(p) => { TILE_URL = 'http://127.0.0.1:' + p + '/{z}/{x}/{y}.png'; }", port)
    page.check("#basemap")
...
with sync_playwright() as pw:
    ...
    p3.evaluate("() => { TILE_URL = 'http://127.0.0.1:1/{z}/{x}/{y}.png'; }")
    p3.check("#basemap"); p3.wait_for_timeout(7500)
    ...
    check("nedostupný server: aplikace to oznámí",
          "nepodařilo" in p3.text_content("#toast"), p3.text_content("#toast"))
    p3.close()
    b.close()
